...
class EmbeddingCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"documents_collection_id_only_embedding"
...
class EmbeddingCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"documents_collection_embedding"
...
class EmbeddingConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    
    @property
    def nome_do_grupo(self):
        return f"Embedding_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = Embedding.objects.filter(pk=self.object_pk, criado_por=user)
