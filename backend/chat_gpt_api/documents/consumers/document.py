...
class DocumentCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"documents_collection_id_only_document"
...
class DocumentCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"documents_collection_document"
...
class DocumentConsumer(AsyncJsonWebsocketConsumer):
    ...
    @property
    def nome_do_grupo(self):
        return f"Document_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = Document.objects.filter(pk=self.object_pk, criado_por=user)
