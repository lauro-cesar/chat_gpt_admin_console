...
def on_create_embedding_index_task(object_pk):
    ...
    if instance:
        if instance.isReadyForIndex and not instance.isIndexed:
            key = f"{instance.document.knowledge_base.vector_prefix}:{str(instance.id)}"
            try:
                generated_embedding = np.array(instance.generated_embedding.get("data",{})[0].get("embedding"), dtype=np.float32).tobytes()  
                ...
                redis_client.hset(key,mapping={
                    "embedding_raw_content":instance.embedding_raw_content,
            ...
            logger.info(f"Criado indice para {instance.document_page}")
...
def on_create_embedding_task(object_pk):
    ...
    instance = Embedding.get_or_none(pk=object_pk)
    if instance:
        logger.info(f"Criado incorporacao para {instance}")
        isReadyForIndex = False 
        generated_embedding = {}
        embedding_content_tokens = token_encoding.encode(instance.embedding_raw_content)
        try:
            openai.api_key = instance.document.knowledge_base.organization.chatgpt_api_token                   
            instance.num_tokens = len(embedding_content_tokens)
            # instance.embedding_content_tokens = embedding_content_tokens                          
