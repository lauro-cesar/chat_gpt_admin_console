...
class KnowledgeBaseCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"organizations_collection_id_only_knowledgebase"
...
class KnowledgeBaseCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"organizations_collection_knowledgebase"
...
class KnowledgeBaseConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    
    @property
    def nome_do_grupo(self):
        return f"KnowledgeBase_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = KnowledgeBase.objects.filter(pk=self.object_pk, criado_por=user)
