...
class PromptTemplateCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"chats_collection_id_only_prompttemplate"
...
class PromptTemplateCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"chats_collection_prompttemplate"
...
class PromptTemplateConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    
    @property
    def nome_do_grupo(self):
        return f"PromptTemplate_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = PromptTemplate.objects.filter(pk=self.object_pk, criado_por=user)
