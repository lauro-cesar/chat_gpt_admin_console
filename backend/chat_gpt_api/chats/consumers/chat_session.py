...
class ChatSessionCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"chats_collection_id_only_chatsession"
...
class ChatSessionCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"chats_collection_chatsession"
...
class ChatSessionConsumer(AsyncJsonWebsocketConsumer):
    ...
    @property
    def nome_do_grupo(self):
        return f"ChatSession_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = ChatSession.objects.filter(pk=self.object_pk, criado_por=user)
