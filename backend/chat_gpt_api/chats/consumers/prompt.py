...
class PromptCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):  
    @property
    def nome_do_grupo(self):
        return f"chats_collection_id_only_prompt"
...
class PromptCollectionConsumer(AsyncJsonWebsocketConsumer):   
    
    @property
    def nome_do_grupo(self):
        return f"chats_collection_prompt"
...
class PromptConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    
    @property
    def nome_do_grupo(self):
        return f"Prompt_{self.object_pk}" 
    ...
    def can_view(self):
        ...
        try:
            ...
            records = Prompt.objects.filter(pk=self.object_pk, criado_por=user)
