...
class AccountTypeCollectionIdOnlyConsumer(AsyncJsonWebsocketConsumer):
    @property
    def nome_do_grupo(self):
        return f"accounts_collection_id_only_accounttype"
...
class AccountTypeCollectionConsumer(AsyncJsonWebsocketConsumer):
    @property
    def nome_do_grupo(self):
        return f"accounts_collection_accounttype"
...
class AccountTypeConsumer(AsyncJsonWebsocketConsumer):
    object_pk = -1

    @property
    def nome_do_grupo(self):
        return f"AccountType_{self.object_pk}"
    ...
    def can_view(self):
        ...
        try:
            logger.info(
                f"Check if user can connect to socket, object_pk= {self.object_pk}"
            )
            ...
            records = AccountType.objects.filter(pk=self.object_pk, criado_por=user)
